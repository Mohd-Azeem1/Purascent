  ...
  return (
    <header className='w-full z-[99] h-[2vh] md:h-[10vh] max-md:h-10 fixed text-[#F8CBFF] backdrop-blur-[3vh]  justify-between items-center px-12 max-md:px-2 py-8 flex junge'>
      ...
      <div className='w-[10vw] flex'>
          <Link className='hover:tracking-widest hover:scale-[1.05] transition-all text-2xl max-md:text-sm  justify-center items-center shadow-[#F8CBFF]' to={"/"}>PURASCENT</Link>
      </div>
     ...
     <ul className={`sidebar md:hidden ${!openOrClose && 'hidden'} w-[40vw] h-screen flex-col fixed top-0 left-0 justify-start bg-[#3A0D4F] p-5 gap-[2vw] tracking-wider shadow-[_10px_10px_10px_rgba(0,0,0,0.3)]`}>
      ...
      {navItems.map((item)=> (<li onClick={()=>{close()}} className='hover:tracking-widest hover:scale-[1.1] hover:border-[1px] hover:border-[#F8CBFF] px-2 py-1 transition-all rounded-full' key={item.name}><Link to={item.slug}>{item.name}</Link></li>))}
     </ul>
